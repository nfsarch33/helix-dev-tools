...
func TestMemoryLayerHelpersAndSummary(t *testing.T) {
	...
	if layer, op, result := InferMemoryContextFromReadPath("/home/testuser/Code/global-kb/global-memories/daily-startup-prompt.md"); layer != MemoryLayerGitKB || op != MemoryOpRead || result != MemoryResultHit {
	...
	events := []Event{
		...
		{Timestamp: now.Add(-2 * time.Minute), Hook: "sanitize-read", Action: "allow", Category: "tool", Detail: "/home/testuser/Code/global-kb/sop/mcp-tools-reference.md", MemoryLayer: MemoryLayerGitKB, MemoryOp: MemoryOpRead, MemoryResult: MemoryResultHit},
